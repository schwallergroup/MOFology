...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "studies", "results"/ML_Chem/transe_results)

os.makedirs(OUTPUT_DIR, exist_ok=True)

def main():
    logging.info("Loading chemical properties and TransE embeddings...")
    df_prop = pd.read_csv(DATA_PATH)
    df_emb = pd.read_csv(TRANSE_PATH)
    
    # Merge on mof_uri
    df = pd.merge(df_prop, df_emb, on='mof_uri', how='inner')
    logging.info(f"Merged data size: {df.shape}")
    
    # Identify targets (numeric columns with enough data)
    ...
    for target in target_cols:
        ...
        logging.info(f"Processing target: {target} ({mask.sum()} samples)")
    ...
    results_df.to_csv(f"{OUTPUT_DIR}/transe_chem_results.csv", index=False)
    
    # Visualization: Heatmap of R2 scores
    pivot_df = results_df.pivot(index="Target", columns="Model", values="R2")
    plt.figure(figsize=(12, 8))
    sns.heatmap(pivot_df, annot=True, cmap="YlGnBu", fmt=".2f")
    plt.title("TransE Embeddings: Chemical Property Prediction (R2)")
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/transe_chem_heatmap.png")
